...
import { UserOutlined, MailOutlined, CommentOutlined, RocketOutlined, CheckCircleOutlined } from '@ant-design/icons';
...
const AttendanceForm = () => {
  ...
  const { setLoading,  submitProgress, setSubmitProgress, showGuide, setShowGuide } = useStore();
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      const fullName = `${values.firstName} ${values.lastName}`.toLowerCase().trim();
      ...
      const userDocRef = doc(db, "users", (values.email || 'default').replace(/[^a-zA-Z0-9]/g, '_'));
      const docSnap = await getDoc(userDocRef);
      const existingSubmissions = docSnap.exists() ? docSnap.data().submissions || [] : [];
      ...
      await runTransaction(db, async (transaction) => {
        // Create explicit document reference
        const submissionId = `${fullName}-${today}`.replace(/[^a-zA-Z0-9-]/g, '');
        const docRef = doc(db, 'submissions', submissionId);
        
        // Direct document existence check
...
style={{ 
  ...
  background: 'linear-gradient(145deg, rgba(16,18,35,0.9) 0%, rgba(23,25,50,0.9) 100%)',
  borderRadius: '20px',
  border: '1px solid rgba(90, 200, 250, 0.3)',
  boxShadow: '0 0 50px rgba(90, 200, 250, 0.2), 0 8px 32px rgba(0, 0, 0, 0.3)',
      ...
      >
        ...
        <Form.Item
          ...
          rules={[
            ...
            { pattern: /^[a-zA-Z0-9]+@gmail\.com$/i, message: 'Invalid Gmail format' }
          ]}
        >
          <Input 
            style={inputStyle}
        ...
        <Form.Item
          name="comments"
          label={<span style={{ color: '#5ac8fa', fontWeight: 500 }}>COMMENTS</span>}
        >
          <TextArea
            ref={fieldRef}
            rows={4}
